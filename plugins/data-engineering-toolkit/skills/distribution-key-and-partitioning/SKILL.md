---
name: distribution-key-and-partitioning
description: >-
  Choose how a large table is spread across nodes and split into partitions in
  an MPP warehouse or distributed store, and fix the skew and data movement
  that make queries slow. Use whenever someone asks which distribution key,
  DISTKEY, DISTRIBUTED BY, hash distribution, sort key, partition key, shard
  key or clustering key to use; whether to hash, round-robin or replicate a
  table; why one node or partition is much bigger or slower than the rest; why
  joins shuffle or broadcast huge amounts of data; or how to partition a big
