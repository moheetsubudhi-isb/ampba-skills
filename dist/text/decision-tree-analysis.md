...
Use this skill when: Structure a one-off business choice under uncertainty as a decision tree (decision nodes, chance nodes, payoffs) and find the best strategy by expected monetary value and backward induction. Use whenever someone must choose between options whose payoff depends on uncertain outcomes: launch or not, bid or not, settle or go to court, build big or small, drill, invest, expand; asks for expected value or EMV; wants to know how sensitive the choice is to a probability or payoff, or the breakeven probability where the decision flips; or whether risk aversion, a utility function or a worst-case view should change the answer. Not for decision tree, random forest or boosting models trained on data, not for pricing a test, survey or pilot before deciding, and not for simulating many uncertain inputs.
